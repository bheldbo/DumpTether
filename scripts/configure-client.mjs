...
const plannedWrites = [
  ...
  updateJsonFile(tauriConfigPath, (tauriConfig) => {
    ...
    tauriConfig.app.security.csp = renderDesktopCsp(target.cloudApiBaseUrl);
...
function renderDesktopCsp(cloudApiBaseUrl) {
  ...
  if (cloudApiBaseUrl) {
    const cloudOrigin = new URL(cloudApiBaseUrl).origin;
    connectSources.push(cloudOrigin);

    const websocketOrigin = new URL(cloudOrigin);
